...
const createNew = async (req, res, next) => {
  ...
  const correctCondition = Joi.object({
    title: Joi.string().required().min(3).max(50).trim().strict().messages({
      'any.required': 'Title is required (HDK)',
      'string.empty': 'Title is not allowed to be empty (HDK)',
      'string.min': 'Title length must be at least 3 characters long (HDK)',
      'string.max': 'Title length must be less than or equal to 50 characters long (HDK)',
      'string.trim': 'Title must not have leading or trailing whitespace (HDK)'
    }),
